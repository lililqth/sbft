...
void ReplicaImp::tryToSendPrePrepareMsg(bool batchingLogic) {
  ...
  LOG_INFO_F(GL,
             "in trytosendpreparemsg 1 size=%d",
             (int)requestsQueueOfPrimary.size());
  if (requestsInQueue == 0) return;
  LOG_INFO_F(GL,
             "in trytosendpreparemsg 2 size=%d",
             (int)requestsQueueOfPrimary.size());
  ...
  LOG_INFO_F(GL,
             "in trytosendpreparemsg 3 size=%d",
             (int)requestsQueueOfPrimary.size());
  ...
  LOG_INFO_F(GL,
             "in trytosendpreparemsg 4 size=%d",
             (int)requestsQueueOfPrimary.size());
  ...
  while (nextRequest != nullptr && reqsInBatch == 0 &&
    ...
    nextRequest =
        (requestsQueueOfPrimary.size() > 0 ? requestsQueueOfPrimary.front()
                                           : nullptr);
    LOG_INFO_F(GL,
               "in trytosendpreparemsg 5 size=%d",
               (int)requestsQueueOfPrimary.size());
  }
...

{
  ...
  metricsTimer_ = new Timer(
      timersScheduler, 100, metricsTimerHandlerFunc, (InternalReplicaApi*)this);
...
void ReplicaImp::executeRequestsInPrePrepareMsg(PrePrepareMsg* ppMsg) {
  ...
  if (firstCommitPathChanged) {
    metric_first_commit_path_.Get().Set(
